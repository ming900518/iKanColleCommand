...
extension AppearanceVC: UITableViewDelegate {
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                let dialog = UIAlertController(title: "请选择样式", message: nil, preferredStyle: .actionSheet)
                ...
                dialog.addAction(UIAlertAction(title: "预设", style: .default) { action in
                    Setting.saveUseTheme(value: 0)
                    let result = UIAlertController(title: "请重启App以套用", message: nil, preferredStyle: .alert)
                    result.addAction(UIAlertAction(title: "确定", style: .default, handler: nil))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                ...
                cell.textLabel?.text = "辅助程式主题"
                if Setting.getUseTheme() == 0 {
                    cell.detailTextLabel?.text = "预设"
                } else {
            ...
            if (indexPath.row == 0) {
                let cell = UITableViewCell(style: .value1, reuseIdentifier: cellIdentifier)
                cell.textLabel?.text = "辅助程式弹出速度（数值越小越快）"
